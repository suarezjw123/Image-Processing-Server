...
#include <signal.h>
#include <assert.h>
#define FLAG_FILE "executed.flag"
...
sem_t * imgarr_mutex; // image array mutex
sem_t * arr_mutex; // arrival mutex
sem_t * img_mutex; // image mutex
sem_t * sock_mutex; // conn socket mutex

int * arr = NULL; // arrived index
int * current_op  = NULL;  // current operation index
...
void register_new_image(int conn_socket, struct request * req)
{
	...
	// registering the arrived image by setting it to 0
	arr = realloc(arr, image_count * sizeof(int));
	arr[image_count - 1] = 0;

	// registering the current operation of the arrived image by setting it to 0
	current_op = realloc(current_op, image_count * sizeof(int));
	current_op[image_count - 1] = 0;
...
int worker_main (void * arg)
{
	...
	while (!params->worker_done) {
		...
		sem_wait(&arr_mutex[img_id]);
		int current_arr = arr[img_id];
		arr[img_id]++;
		sem_post(&arr_mutex[img_id]);
		
		sem_wait(&imgarr_mutex[img_id]);

		while (current_op[img_id] != current_arr) {
			sem_post(&imgarr_mutex[img_id]);
			sem_wait(&imgarr_mutex[img_id]);
		}
...
int main (int argc, char ** argv) {
	...
	char *programPath = "./build/server_mimg";
    char** programArgs = argv;

	  // Check if the flag file exists
    if (access(FLAG_FILE, F_OK) == -1) {
        // The flag doesn't exist
        printf("Program hasn't repeated yet. creating new makefile.\n");
		...
		// Execv to start new server
		// Create the flag file to indicate that the program has been executed
		FILE *flagFile = fopen(FLAG_FILE, "w");
		if (flagFile != NULL) {
			fclose(flagFile);
		}
		...
		printf("makefile has already been changed, going onto the main server code\n");
	};
	...
	/* Initialize queue protection variables and other semaphores. DO NOT TOUCH. */
	...
	sock_mutex = (sem_t *)malloc(sizeof(sem_t)); // Initializing socket mutex semaphore
	retval = sem_init(sock_mutex, 0, 1);
	if (retval < 0) {
		ERROR_INFO();
		perror("Unable to initialize connection socket mutex");
		return EXIT_FAILURE;
	}
	img_mutex = (sem_t*)malloc(sizeof(sem_t)); // Initializing the image mutex semaphore
	retval = sem_init(img_mutex, 0, 1);
	if (retval < 0) {
		ERROR_INFO();
		perror("Unable to initialize image register mutex");
		return EXIT_FAILURE;
	}
	imgarr_mutex = (sem_t *)malloc(200 * sizeof(sem_t)); // intializing the image array semaphore mutex
	arr_mutex = (sem_t *)malloc(200 * sizeof(sem_t)); // intializing the arrival semaphore mutex
	for (int i = 0; i < 200; i++) {
		sem_init(&imgarr_mutex[i], 0, 1);
		sem_init(&arr_mutex[i], 0, 1);
	}
